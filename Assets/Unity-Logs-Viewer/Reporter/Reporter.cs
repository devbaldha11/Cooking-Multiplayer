...
public class Reporter : MonoBehaviour
{
    ...
    bool isGestureDone()
    {
            ...
            Application.platform == RuntimePlatform.IPhonePlayer)
        {
            ...
            {
                if (UnityEngine.Input.touches[0].phase == TouchPhase.Canceled || UnityEngine.Input.touches[0].phase == TouchPhase.Ended)
                    gestureDetector.Clear();
                else if (UnityEngine.Input.touches[0].phase == TouchPhase.Moved)
                {
                    Vector2 p = UnityEngine.Input.touches[0].position;
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    Vector2 p = new Vector2(UnityEngine.Input.mousePosition.x, UnityEngine.Input.mousePosition.y);
                    if (gestureDetector.Count == 0 || (p - gestureDetector[gestureDetector.Count - 1]).magnitude > 10)
                        gestureDetector.Add(p);
                }
            }
    ...
    Vector2 getDownPos()
    {
           ...
           Application.platform == RuntimePlatform.IPhonePlayer)
        {
            ...
            if (UnityEngine.Input.touches.Length == 1 && UnityEngine.Input.touches[0].phase == TouchPhase.Began)
            {
                downPos = UnityEngine.Input.touches[0].position;
                return downPos;
            }
        }
        else
    ...
    Vector2 getDrag()
    {
            ...
            Application.platform == RuntimePlatform.IPhonePlayer)
        {
            ...
            return UnityEngine.Input.touches[0].position - downPos;
        }
        else
        {
            if (UnityEngine.Input.GetMouseButton(0))
